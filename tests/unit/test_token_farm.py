...
import pytest
from scripts.deploy import KEPT_BALANCE, deploy_token_farm_and_golden_token
...
def test_set_price_feed_contract():
    ...
    token_farm.setPriceFeedContract(
        golden_token.address, price_feed_address, {"from": account}
    )

    # Assert
    assert token_farm.tokenPriceFeedMapping(golden_token.address) == price_feed_address
    with pytest.raises(exceptions.VirtualMachineError):
        token_farm.setPriceFeedContract(
            golden_token.address, price_feed_address, {"from": non_owner}
        )
...
def test_stake_tokens(amount_staked):
    ...
    # Assert
    assert (
        token_farm.stakingBalance(golden_token.address, account.address)
        == amount_staked
    )
...
def test_issue_tokens(amount_staked):
    ...
    # Arrange
    # we are staking 1 golden_token == in price to 1 ETH
    # soo... we should get 2,000 golden tokens in reward
...
def test_get_token_value():
    ...
    token_farm, golden_token = deploy_token_farm_and_golden_token()
    # Act / Assert
    assert token_farm.getTokenValue(golden_token.address) == (
        INITIAL_PRICE_FEED_VALUE,
        decimals,
    )
...
def test_token_is_allowed():
    ...
    # Assert
    assert token_farm.tokenIsAllowed(golden_token.address) == True
